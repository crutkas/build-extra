...
def strict_decode(raw, encoding, code="INVALID_PATH_ENCODING"):
    try:
...
def normalize_path(text, is_directory, max_length):
    if "\0" in text or any(ord(char) < 0x20 or ord(char) == 0x7f for char in text):
        reject("UNSAFE_PATH", "Archive path contains a control character", path=text)
...
def normalize_link_target(member_path, target, max_length, root_relative=False):
    if "\0" in target or any(ord(char) < 0x20 or ord(char) == 0x7f for char in target):
        reject("UNSAFE_LINK_TARGET", "Link target contains a control character", path=member_path, target=target)
...
class ArchiveAuditor:
    ...
    def _audit_bytes(self, data, name, parent, depth, chain):
        ...
        expected = expected_format(name)
        detected, signature_offset = detect_format(data, name)
        if detected and expected and detected != expected:
            reject(
                "NESTED_FORMAT_DISAGREEMENT" if parent else "FORMAT_DISAGREEMENT",
                "Archive extension and physical format disagree",
                name=name,
                expected=expected,
                detected=detected,
            )
        archive_format = detected or expected
        if not archive_format:
            reject("UNRECOGNIZED_ARCHIVE", "Input is not a supported archive format", name=name)
        ...
        if archive_format == "zip":
            ...
            self._parse_wrapper(data, name, node, archive_format)
        elif archive_format == "7z":
            self._parse_7z(data, name, node, signature_offset)
        else:
    ...
    def _attach_nested(self, node, depth, chain):
        for member in node["members"]:
            ...
            if member["type"] != "file":
                continue
            nested_expected = expected_format(nested_name)
            nested_detected, _ = detect_format(content, nested_name)
            if nested_expected and nested_detected and nested_expected != nested_detected:
                reject(
                    "NESTED_FORMAT_DISAGREEMENT",
                    "Nested member extension and physical format disagree",
                    path=nested_name,
                    expected=nested_expected,
                    detected=nested_detected,
                )
            if not nested_expected and not nested_detected:
                continue
            parent = {
    ...
    def _parse_wrapper(self, data, name, node, archive_format):
        if archive_format == "gzip":
            ...
            if consumed != len(data):
                reject("TRAILING_COMPRESSED_PAYLOAD", "bzip2 stream has trailing bytes", offset=consumed)
            data_offset = 4
            ...
            if consumed != len(data):
                reject("TRAILING_COMPRESSED_PAYLOAD", "xz stream has trailing bytes", offset=consumed)
            if len(data) < 24:
            ...
            if consumed != len(data):
                reject("TRAILING_COMPRESSED_PAYLOAD", "zstd stream has trailing bytes", offset=consumed)
            stored_length = len(data) - data_offset
            owner = dict(zstd_owner, streamEndOffset=consumed)
            embedded_name = None
            embedded_name_raw = None

        derived_payload_name = wrapper_payload_name(name)
        ...
        if embedded_name:
            ...
            embedded_base = embedded_name.rsplit("/", 1)[-1]
            derived_base = (
                derived_payload_name.rsplit("/", 1)[-1]
                if derived_payload_name
                else None
            )
            if derived_base and embedded_base != derived_base:
                reject(
                    "AMBIGUOUS_WRAPPER_NAME",
                    "gzip embedded name disagrees with its containing archive name",
                    embeddedName=embedded_name,
                    derivedName=derived_payload_name,
                )
            payload_name = embedded_base
        ...
        member["_nestedName"] = derived_payload_name or payload_name
    ...
    def _decode_gzip(self, data):
        ...
        if flags & 0x08:
            ...
            embedded_name = strict_decode(embedded_name_raw, "latin-1", "INVALID_GZIP_NAME")
            cursor = end + 1
        comment_raw = None
        if flags & 0x10:
            end = data.find(b"\0", cursor)
            ...
            strict_decode(comment_raw, "latin-1", "INVALID_GZIP_COMMENT")
            cursor = end + 1
        if flags & 0x02:
        ...
        if footer_offset + 8 != len(data):
            reject("TRAILING_COMPRESSED_PAYLOAD", "gzip stream has a concatenated stream or trailing bytes", offset=footer_offset + 8)
    ...
    def _parse_7z(self, data, name, node, signature_offset):
        if signature_offset and not (name.lower().endswith(".7z.exe") and data.startswith(b"MZ")):
            reject("UNSUPPORTED_7Z_PREFIX", "Only MZ-prefixed .7z.exe SFX archives may precede a 7z signature")
        signature = checked_slice(data, signature_offset, 32)
        if signature[:6] != SEVEN_ZIP_SIGNATURE:
            reject("INVALID_7Z_SIGNATURE", "7z signature is invalid")
        ...
        next_start = signature_offset + 32 + next_offset
        next_header = checked_slice(data, next_start, next_size)
        if next_start + next_size != len(data):
            reject(
                "TRAILING_7Z_PAYLOAD",
                "7z next header does not end at the archive boundary",
                nextHeaderEnd=next_start + next_size,
    ...
    def _seven_decode_coder(self, coder, packed, expected_size):
        ...
        if len(expanded) != expected_size or decoder.unused_data:
            reject(
...
def expected_format(name):
    lower = name.lower()
    if lower.endswith((".zip", ".nupkg", ".msix", ".vsix")):
        return "zip"
    if lower.endswith(".7z") or lower.endswith(".7z.exe"):
        return "7z"
    if lower.endswith((".tar.gz", ".tgz", ".src.gz")) or lower.endswith(".gz"):
        return "gzip"
    if lower.endswith((".tar.bz2", ".tbz2")) or lower.endswith(".bz2"):
        return "bzip2"
    if lower.endswith((".tar.xz", ".txz")) or lower.endswith(".xz"):
        return "xz"
    if lower.endswith((".tar.zst", ".tzst")) or lower.endswith((".zst", ".zstd")):
        return "zstd"
...
def detect_format(data, name=""):
    ...
    if data.startswith(ZSTD_SIGNATURE):
        return "zstd", 0
    if len(data) >= 512 and data[257:262] in (b"ustar",):
        return "tar", 0
    if name.lower().endswith(".7z.exe"):
        first = data.find(SEVEN_ZIP_SIGNATURE)
        if first >= 0:
            second = data.find(SEVEN_ZIP_SIGNATURE, first + 1)
            if second >= 0:
                reject(
                    "AMBIGUOUS_7Z_SIGNATURE",
                    "SFX contains multiple candidate 7z signatures",
                    firstOffset=first,
                    secondOffset=second,
                )
            return "7z", first
...
def wrapper_payload_name(name):
    lower = name.lower()
    suffixes = (".gz", ".bz2", ".xz", ".zst", ".zstd")
